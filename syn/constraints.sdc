#------------------------------------------------------------------------------
# constraints.sdc -- synthesis/PnR timing constraints for dac_demdrz_top
#
...
# receiver and the sw_* flops are placed adjacent to the switch drivers.
#------------------------------------------------------------------------------
...
# Asynchronous reset: constrain recovery/removal via a false path on assert,
# synchronize deassertion externally.
set_false_path -from [get_ports rst_n]

# Quasi-static configuration inputs (changed only between bursts).
set_false_path -from [get_ports dem_en]
set_false_path -from [get_ports drz_en]

# Data interface budget: half a cycle for external launch, half for capture.
set_input_delay  -clock clk [expr {0.50 * $CLK_PERIOD_NS}] [get_ports data_in]
set_output_delay -clock clk [expr {0.25 * $CLK_PERIOD_NS}] [get_ports data_req]

# The 28 switch-control lines and the phase indicator drive the full-custom
...
set_output_delay -clock clk [expr {0.25 * $CLK_PERIOD_NS}] \
    [get_ports {sw_msb[*] sw_ulsb[*] sw_lsb[*] sw_llsb[*] phase}]
